...
char *netname;
{
  ...
  if ((net = bpnn_read(netname)) == NULL) {
    if (train_n > 0) {
      ...
      /* bthom ===========================
	make a net with:
	  imgsize inputs, 20 hidden units, and 1 output unit
          */
      net = bpnn_create(imgsize, 20, 1);
    } else {
...
double *err;
{
  ...
  for (int i = 1; i <= n; i++) 
  {
    ...
    // The following if-statemnet finds the max value and assigns it to the target_value variable
    if(net->target[i]>target_value) 
    {
      sign = i;
      target_value = net->target[i];
    }
  }
  
  *err = (0.5 * delta * delta);

  // check of the max-picked target and of the output units
  if(target_value > 0.5) 
  {
    if(net->output_units[sign] > 0.5) 
    {
      return (1);
    } 
    else 
